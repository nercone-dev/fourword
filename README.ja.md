...
バイト列をビッグエンディアンの符号なし整数として解釈し、`2^N`（Nは全体のビット数）を加算して10進数に変換します。

```
Decimal = 2^N + (バイト列をビッグエンディアンの符号なし整数として解釈した値)
```

結果は同一ビット数では常に固定桁数になるため、デコード時に桁数だけでビット数を特定できます。

**デコード**

桁数からNを特定し、`2^N`を減算することで元のバイト列を復元します。
...
|---:|---:|:---|:---|
| 256 | 64 bit | 約 1.84 × 10¹⁰ 秒 | 約 2554年 |
| 512 | 128 bit | 約 3.40 × 10²⁹ 秒 | 約 1.08 × 10²² 年後 |
...
| 1792 | 448 bit | 約 7.27 × 10¹²⁵ 秒 | 約 2.30 × 10¹¹⁸ 年後 |
| 2048 | 512 bit | 約 1.34 × 10¹⁴⁵ 秒 | 約 4.25 × 10¹³⁷ 年後 |
